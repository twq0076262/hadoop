...
*   ContainerManager: 接收RPC请求（启动、停止），资源本地化（下载应用需要的资源到本地，根据需要共享这些资源）
    ...
    PUBLIC: <local-dir>/filecache</local-dir>

    PRIVATE: <local-dir>/usercache/<username>/filecache</username></local-dir>

    APPLICATION: <local-dir>/usercache/<username>/appcache/<app-id>/（在程序完成后会被删除）</app-id></username></local-dir>
